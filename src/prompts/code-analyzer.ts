...
const codeAnalyzerSchema = {
	...
	analysisType: completable(
		...
		(value) => {
			return (
				["security", "performance", "style", "bugs", "all"] as const
	...
	verbose: completable(
		z
			.enum(["yes", "no"])
			.describe("Include detailed explanations (yes/no)"),
		(value) => {
			return (["yes", "no"] as const).filter((opt) =>
				opt.startsWith(value.toLowerCase())
			);
		}
	),
} as const;
	...
	register(server: McpServer) {
		server.registerPrompt(
			"code-analyzer",
			{
				title: "Code Analyzer",
				description:
					"Analyze code for security, performance, style issues, and bugs",
