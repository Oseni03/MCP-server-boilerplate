...
describe("Generate README Prompt Tests", () => {
	it("should list generate-readme prompt", async () => {
		await withTestClient(async (client) => {
			...
			const readmePrompt = response.prompts.find(
				(p) => p.name === "generate-readme"
			);

			assert(
				readmePrompt !== undefined,
				"Generate README prompt should be listed"
			);
			assert(
	...
	it("should handle concurrent prompt requests", async () => {
		await withTestClient(async (client) => {
			...
			results.forEach((result, index) => {
				...
				assert(
					content !== undefined,
					`Result ${String(index)} should have content`
				);
				...
				if (firstContent.type === "text" && content.type === "text") {
					assert.strictEqual(
						...
						`Result ${String(
							index
						)} should have identical prompt text`
					);
				}
			});
		});
