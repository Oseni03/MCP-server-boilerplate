...
import { withTestClient, assertJSONResource } from "./helpers/test-client.ts";
...
describe("System Info Resource Tests", () => {
	it("should list system-info resource", async () => {
		await withTestClient(async (client) => {
			...
			const systemInfoResource = response.resources.find(
				(r) => r.uri === "system://info"
			);

			assert(
				systemInfoResource !== undefined,
				"System info resource should be listed"
			);
		...
		await withTestClient(async (client) => {
			...
			const systemInfo = assertJSONResource<SystemInfo>(
				...
				(data) => {
					...
					assert(
						typeof data.totalMemory === "number",
						"Total memory should be a number"
					);
					assert(
						typeof data.freeMemory === "number",
						"Free memory should be a number"
					);
				}
			);
	...
	it("should return consistent system info across multiple calls", async () => {
		await withTestClient(async (client) => {
			...
			const systemInfo1 = assertJSONResource<SystemInfo>(
				response1,
				"system://info"
			);

			const response2 = await client.readResource("system://info");
			const systemInfo2 = assertJSONResource<SystemInfo>(
				response2,
				"system://info"
			);
			...
			assert.strictEqual(
				systemInfo1.architecture,
				systemInfo2.architecture
			);
			assert.strictEqual(
				systemInfo1.nodeVersion,
				systemInfo2.nodeVersion
			);
			assert.strictEqual(
				systemInfo1.totalMemory,
				systemInfo2.totalMemory
			);

			// Dynamic values might change but should be reasonable
			assert(
				Math.abs(systemInfo2.uptime - systemInfo1.uptime) < 2,
				"Uptime difference should be small"
			);
		});
	});
		...
		await withTestClient(async (client) => {
			...
			results.forEach((result) => {
				...
				assert(
					...
					"Each result should have valid system info"
				);
			});
		});
	});
